...
# img = board.draw((1000, 1200))
# cv2.imshow("aruco", img)
# cv2.waitKey(0)  # Wait until any key is pressed
# cv2.destroyAllWindows()  # Close all OpenCV windows
# image_path = "aruco_board2.png"
# cv2.imwrite(image_path, img)
# print(f"ArUco marker board saved as {image_path}")
...
# path = "/home/mustafa/urc/aruco_data"
# count = 
...
if calibrate_camera == False:
    ...
    for im in tqdm(img_list):
        ...
        if ids is not None:  # Check if ids is not None before processing

         if first == True:
            corners_list = np.array(corners).reshape(-1, 2)
            id_list = ids
            first = False
        # else:
        ...
        #   id_list.append(ids)
            ...
            corners_list = np.vstack((corners_list, np.array(corners).reshape(-1, 2)))
